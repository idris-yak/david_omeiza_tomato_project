...
def load_model():
    model_path = "/content/drive/MyDrive/Colab Notebooks/Saved_models/tomato_model.keras"
...
# Function to predict and get disease solution
...
if st.session_state.logged_in:
    ...
    if app_mode == "Logout":
       ...
       # Set a background image or color to make the section visually appealing
        st.markdown(
            """
            <style>
            .home-section {
        ...
        if image is not None:
            if st.button('Predict'):
                with st.spinner('Analyzing the image...'):
                    ...
                    st.write(f"**Disease Solution:**\n{disease_solution}")

                    # Display the image with prediction
                    st.image(image, caption='Processed Image', use_column_width=True)

                    # Optionally, you can plot additional insights here if needed

        else:
            st.info("Please capture or upload an image to proceed with the prediction.")
    ...
    st.sidebar.markdown(
        """
        **Note:** To access the full features of the Tomato Plant Disease Classification System,
        please log in or register if you haven't already.
        """
    )
